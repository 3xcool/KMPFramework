...
- ✅ Multi-action DS components — per-target `analyticsId` + shared `analyticsParams` on `DsAlert` (`dismissAnalyticsId` → `ds_alert_dismissed`), `DsBanner` (`primary` / `secondary` / `dismiss` → three distinct `ds_banner_*` events), `DsDialogWeb` + `DsDialogMaterial` (`confirm` / `cancel` / `dismiss` → three distinct `ds_dialog_*` events), and `DsSnackbar` (`analyticsId` on `DsSnackbarMessage` → `ds_snackbar_action_clicked`). Each event includes `id` plus component-specific context (`type`, `label`, `title`). Backwards-compatible — every new param defaults to `null` / `emptyMap`. Full event table in [docs/analytics.md](analytics.md). Clickable `DsCard` is single-action and only needs the existing `analyticsId` param when it lands.
- ✅ Kompass screen-view auto-tracking — `NavController.screenChanges(): Flow<ScreenChange>` in `:framework:kompass` (snapshotFlow over the top destinationId with distinctUntilChanged + filterNotNull so empty back stacks don't emit malformed events). `TrackScreenViews(navController)` composable in `:framework:sdk` wires it to `Framework.analytics` — drop once next to `NavigationHost` and every destination change calls `analytics.screen(destinationId, mapOf("from" to previous))`. 4/4 tests passing in `:framework:kompass:jvmTest` covering initial emission, navigate, distinctUntilChanged on same destinationId, and pop.
